...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // //Esto selecciona el genero
  const queryString = window.location.search;
   let genero = queryString.substring( queryString.indexOf('=') + 1 );
   console.log(genero); 
   CambiarTitulo();

    const API_POPULARES_URL = BASE_URL + '/discover/movie?sort_by=popularity.desc&' + API_KEY + '&language=es-ES&page=1';
    const API_GENRES_URL = API_POPULARES_URL +'&with_genres='+genero
  
    crearTarjetas(API_GENRES_URL, 'genero');
